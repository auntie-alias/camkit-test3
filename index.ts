...
import { bootstrapCameraKit, Transform2D } from "./node_modules/@snap/camera-kit/";
...
window.addEventListener("load", async () => {
    try {
        ...
        const cameraKit = await bootstrapCameraKit({ apiToken: "[api token]" });
        ...
        const { lenses } = await cameraKit.lensRepository.loadLensGroups(["[lens group]"]);
/*
